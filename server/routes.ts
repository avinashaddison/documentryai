...
export async function registerRoutes(
  ...
  // Documentary autopilot endpoint (alias for /api/projects/:id/autopilot with documentary path)
  app.post("/api/projects/:id/documentary/autopilot", async (req, res) => {
    try {
      const projectId = parseInt(req.params.id);
      const { chapters, voice = "aura-2-mars-en", imageModel = "flux-1.1-pro", imageStyle = "color" } = req.body;
      ...
      await storage.createGenerationLog({
        ...
        step: "autopilot",
        status: "started",
        message: `Starting autopilot generation: images → audio → video...`
      });

      // Run autopilot in background and return immediately
  ...
  // Documentary status endpoint
  app.get("/api/projects/:id/documentary/status", async (req, res) => {
    try {
      const projectId = parseInt(req.params.id);
      const status = await getGenerationStatus(projectId);
      const logs = await storage.getGenerationLogsByProject(projectId);
      const project = await storage.getProject(projectId);
      const assets = await storage.getGeneratedAssetsByProject(projectId);
      
      // Sort logs by ID and get last 20
      const sortedLogs = logs.sort((a, b) => a.id - b.id).slice(-20);
      
      // Determine video path from completion log or standard path
      let videoPath = null;
      if (status.session?.status === "completed") {
        const completionLog = logs.find(l => l.step === "autopilot" && l.status === "completed" && l.message?.includes("Video saved"));
        if (completionLog && completionLog.message) {
          const match = completionLog.message.match(/saved to (.+)$/);
          if (match) {
            videoPath = `/${match[1]}`;
          }
        }
        if (!videoPath) {
          videoPath = `/generated_videos/project_${projectId}_final.mp4`;
        }
      }
      
      // Calculate totalScenes from assets if not set in session
      const uniqueScenes = new Set(assets.map(a => `${a.chapterNumber}_${a.sceneNumber}`));
      const derivedTotalScenes = status.session?.totalScenes || uniqueScenes.size || 1;
      
      // Calculate overall progress server-side for reliability
      const imageAssets = assets.filter(a => a.assetType === "image" && a.status === "completed");
      const audioAssets = assets.filter(a => a.assetType === "audio" && a.status === "completed");
      let overallProgress = 0;
      
      if (status.session?.status === "completed") {
        overallProgress = 100;
      } else if (status.session?.status === "failed") {
        overallProgress = Math.round(25 + ((imageAssets.length + audioAssets.length) / (derivedTotalScenes * 2)) * 65);
      } else if (derivedTotalScenes > 0) {
        overallProgress = Math.round(25 + ((imageAssets.length + audioAssets.length) / (derivedTotalScenes * 2)) * 65);
        overallProgress = Math.min(overallProgress, 90); // Cap at 90% until complete
      }
      ...
      res.json({
        ...
        session: status.session ? {
          ...status.session,
          totalScenes: derivedTotalScenes,
          errorMessage: status.session.errorMessage || null,
        } : null,
